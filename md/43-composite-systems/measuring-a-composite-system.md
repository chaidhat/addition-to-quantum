...
Measure $\hat{S}_x^{(\text{tot})}$, get 0 with $\frac{1}{2}$ probability. Mathematically, we use a [[projectors|projector]] to measure the state. Via the projection rule we get
