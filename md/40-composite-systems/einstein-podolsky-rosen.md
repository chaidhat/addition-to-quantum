...
Alice now measures her qubit in the z [[basis-state|basis]]. Let’s say she gets $0$.
...
That means, via. Born’s rule, if Alice observed a $0$ outcome, Bob’s qubit is in the [[ket-state|state]] $\left\lvert 1 \right\rangle$ with 100% [[probability-theory|probability]].
...
[[local-realism|Local realism]] assumes that before every measurement, every observable has a pre-existing definite value determined by some hidden [[variable|variable]] $\lambda$. So in any given run, $A_1,A_2,B_1,B_2$ all have predetermined definite values.
