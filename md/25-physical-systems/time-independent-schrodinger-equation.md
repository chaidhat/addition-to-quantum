...
$$
 ...
 $$
If we decompose this as we know any state $\ket{\psi(0)}$ is a superposition
$$
...
\ket{\phi_n}$$
$$
\ket{\psi(t)} = \sum_n c_n(t)\,e^{-iE_n t/\hbar}\ket{\phi_n}
$$
where $c_n(t)$ is how much of $\ket{\phi_n}$ position is in the state $\ket{\psi(t)}$ at time $t$

This is called the Time-Independent Schrödinger Equation (TISE)
$$
