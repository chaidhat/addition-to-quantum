...
Postulates of an [<span class="underline">isolated system</span>](#isolated-system):
...
2.  must preserve [<span class="underline">probablity</span>](#probability-theory) (must sum to 1) if isolated which means $U$ must be [<span class="underline">unitary</span>](#unitary-operator) if [<span class="underline">Hilbert space</span>](#hilbert-spaces) is finite dimensional.

3.  assume $U$ is unitary in infinite dimensional Hilbert space.

Unpacking 2. we see that
$$\left\langle \psi|\psi \right\rangle=1$$
This is because [<span class="underline">Born’s rule</span>](#borns-rule) says that, over $\left\lvert k \right\rangle$ basis states,
$$\sum_k |\left\langle k|\psi \right\rangle|^2=1$$
as probability must sum to 1.
Using [<span class="underline">this lemma</span>](#absolute-value-of-complex-number), we get
...
This means that $U$ is [<span class="underline">unitary</span>](#unitary-operator).
