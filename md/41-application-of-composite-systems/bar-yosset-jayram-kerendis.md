...
In QM, we'd only need to send $O(\log n)$ [[qubit|qubits]]. Alice can encode all of $x$ into amplitudes of a quantum state on only $\log_2 n$ qubits and send that.

Alice prepares
$$
\ket{\psi_x} = \tfrac{1}{\sqrt{n}}\sum_{i=1}^{n} (-1)^{x_i}\ket{i}
$$
Bob measures $\ket{\psi_x}$ in the basis
$$
