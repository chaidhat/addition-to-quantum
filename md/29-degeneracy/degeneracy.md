...
Two or more [<span class="underline">linear independent</span>](#linear-independence) quantum states ([<span class="underline">eigenstates</span>](#eigenstates)) share the same [<span class="underline">eigenvalue</span>](#eigenvectorseigenvalues).
An energy level $E_n$ is degenerate if there exist multiple [<span class="underline">linearly independent</span>](#linear-independence) [<span class="underline">eigenstates</span>](#eigenstates) $\left\lvert \psi_1 \right\rangle, \left\lvert \psi_2 \right\rangle, \ldots, \left\lvert \psi_n \right\rangle$ which satisfy
...
If something is degenerate, that implies there is no unique [<span class="underline">basis</span>](#basis-states). What you want to write down should be a [<span class="underline">projection</span>](#projections) onto the [<span class="underline">subspaces</span>](#subspaces).
...
This just means that you can’t detect the difference between states given only the energy/[<span class="underline">eigenvalue</span>](#eigenvectorseigenvalues).

Remember the [<span class="underline">eigenvalue</span>](#eigenvectorseigenvalues) is observed as energy if the [<span class="underline">operator</span>](#operators) is the [<span class="underline">Hamiltonian</span>](#schruxf6dingers-equation-hamiltonian).
...
BUT if $[\hat{A},\hat{H}]\neq0$ then this doesn’t always hold true for the entire [<span class="underline">Hilbert Space</span>](#hilbert-spaces).
...
This means that $\hat{A}\left\lvert \psi \right\rangle$ is in the $E$-[<span class="underline">eigenspace</span>](#eigenspaces) of $\hat{H}$. Another way to say this is that $\hat{A}\left\lvert \psi \right\rangle$ is in the set of [<span class="underline">eigenvectors</span>](#eigenvectorseigenvalues) for H that share [<span class="underline">eigenvalue</span>](#eigenvectorseigenvalues) E.
